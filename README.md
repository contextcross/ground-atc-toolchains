...
> 
> 支持导入/导出CSV、已实现自动呼号去重、停机位信息输入和检测、一键重新排布、自动等差配置时间、自动检测同一停机位多进近/先进近后起飞等不合法情况。
>
> 支持进近航班、起飞航班、全航班、停机坪4个视图查看。
> 
> **[点击启动时刻表编辑器 ->](https://contextcross.github.io/ground-atc-toolchains/FlightScheduleEditor.html)**
>
> **TODO**
>
> 支持导入OSM信息解析文件，根据停机位大小分配合适的机型。
> 
> 支持中转航班（相同涂装不同呼号）。
> 
> 通过算法量化关卡设计，支持多维度变量的难度精细调整，指定条件自动生成对应难度的排班表，并作为开发无尽模式关卡生成逻辑的基础。
